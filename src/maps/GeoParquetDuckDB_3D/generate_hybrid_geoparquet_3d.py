# generate_hybrid_geoparquet_3d.py
import duckdb
...
OUTPUT_HYBRID_GEOPARQUET_PATH = '/Users/marianakecova/GST/3DFLUS_CCN/UC5_PRAHA_EGMS/t146/SRC_DATA/egms_hybrid_3d.geoparquet'
...
print(f"--- Starting 3D Hybrid GeoParquet Generation ---")

con = duckdb.connect()

try:
    static_cols_to_select = [col for col in STATIC_COLUMNS if col not in ['height', 'height_wgs84']]
    static_cols_str = ", ".join([f'"{col}"' for col in static_cols_to_select])

    query = f"""
    WITH UnpivotedData AS (
        UNPIVOT read_csv_auto('{INPUT_WIDE_CSV_PATH}')
        ON COLUMNS(* EXCLUDE ({", ".join([f'"{c}"' for c in STATIC_COLUMNS])}))
    ...
    SELECT
        ...
        (200 + SUM(displacement)/10) AS height
    FROM UnpivotedData
    ...
    gdf = gpd.GeoDataFrame(
        ...
        geometry=gpd.points_from_xy(
            x=df_hybrid['longitude'],
            y=df_hybrid['latitude'],
            z=df_hybrid['height']
        ),
        crs="EPSG:4326"
    )
    ...
    print(f"Successfully wrote 3D hybrid file to: {OUTPUT_HYBRID_GEOPARQUET_PATH}")
